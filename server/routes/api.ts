...
import { AudioEditService, EDITABLE_FORMATS, parseTimeInput } from '../services/audioEditService.js';
...
function resolveAudioFile(id: string): { filePath: string; fileName: string } | null {
  ...
  if (job?.outputFilePath && fs.existsSync(job.outputFilePath)) {
    return { filePath: job.outputFilePath, fileName: job.outputFileName || path.basename(job.outputFilePath) };
  }
...
function resolveLibraryTarget(id: string): {
  ...
  const job = JobManager.getJob(id);
  const record = LibraryStore.list().find((r) => r.jobId === id);
  const filePath = job?.outputFilePath && fs.existsSync(job.outputFilePath)
    ? job.outputFilePath
...
function syncLibraryIndexes(
  id: string,
  update: { filePath: string; fileName: string; fileSizeBytes: number; format: string; title: string; author: string; thumbnail: string; videoId: string; completedAt: number },
): void {
  const job = JobManager.getJob(id);
  if (job) {
...
apiRouter.post('/convert', async (req: Request, res: Response) => {
  try {
    const { url, format, bitrate, trimStart, trimEnd, volumeBoost, normalizeAudio, embedThumbnail } = req.body;
    if (!url) {
...
apiRouter.patch('/settings', (req: Request, res: Response) => {
  try {
    ...
    const settings = SettingsService.updateSettings({ downloadsDir, filenameTemplate, revealAfterConvert });
...
apiRouter.get('/library', (req: Request, res: Response) => {
  const records = LibraryStore.list();
  const indexedPaths = new Set(records.map((r) => path.normalize(r.filePath)));
  const loose = FileService.scanLibrary().filter((f) => !indexedPaths.has(path.normalize(f.filePath)));
  const totalSizeBytes =
    records.reduce((sum, r) => sum + r.fileSizeBytes, 0) +
    loose.reduce((sum, f) => sum + f.sizeBytes, 0);
  res.json({
    success: true,
    data: {
...
apiRouter.delete('/library/:id', (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({ success: false, error: error.message || 'Could not delete the file.' });
    return;
  }
  LibraryStore.remove(id);
...
apiRouter.get('/library/:id/probe', async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({ success: false, error: error.message || 'Could not probe the audio file' });
  }
});
...
apiRouter.post('/library/:id/trim', async (req: Request, res: Response) => {
  ...
  const startSecs = typeof start === 'string' ? parseTimeInput(start) ?? (start.trim() === '' ? 0 : null) : 0;
  ...
  if (startSecs === null || (typeof end === 'string' && end.trim() !== '' && endSecs === null)) {
    res.status(400).json({ success: false, error: 'Invalid trim times. Use seconds or MM:SS (e.g. 15 or 0:15).' });
    return;
  }
  ...
  try {
    ...
    if (probe.durationSeconds !== null) {
      if ((startSecs ?? 0) >= probe.durationSeconds) {
        res.status(400).json({ success: false, error: 'Trim start is past the end of the track.' });
        return;
      }
      if (endSecs !== null && endSecs > probe.durationSeconds) {
        res.status(400).json({ success: false, error: 'Trim end is past the end of the track.' });
        return;
      }
    }
    ...
    res.status(500).json({ success: false, error: error.message || 'Could not trim the audio file' });
  }
});
...
apiRouter.post('/library/:id/edit', async (req: Request, res: Response) => {
  ...
  const { format, bitrate, normalizeAudio, volumeBoost, title, artist } = (req.body ?? {}) as {
    format?: string; bitrate?: string; normalizeAudio?: boolean; volumeBoost?: number; title?: string; artist?: string;
  };
  ...
  if (!(EDITABLE_FORMATS as readonly string[]).includes(nextFormat)) {
    res.status(400).json({ success: false, error: `Unsupported format. Choose one of: ${(EDITABLE_FORMATS as readonly string[]).join(', ')}.` });
    return;
  }
  ...
  try {
    ...
    const finalName = displayName === currentBase ? currentBase : dedupeFileName(dir, displayName);
    ...
    res.status(500).json({ success: false, error: error.message || 'Could not update the audio file' });
  }
});
...
apiRouter.post('/files/reveal', async (req: Request, res: Response) => {
  try {
    ...
    if (process.platform === 'win32') {
      ...
      spawn('xdg-open', [path.dirname(absolute)], { detached: true, stdio: 'ignore' }).unref();
    }
    res.json({ success: true, path: absolute });
  } catch (error: any) {
    res.status(500).json({ success: false, error: error.message || 'Could not reveal the file.' });
  }
});
...
apiRouter.post('/tags/apply/:id', async (req: Request, res: Response) => {
  try {
    ...
    const target = job?.outputFilePath && fs.existsSync(job.outputFilePath)
      ? { filePath: job.outputFilePath, format: job.format, title: job.title, author: job.author, thumbnail: job.thumbnail, videoId: job.videoId, completedAt: job.completedAt ?? Date.now() }
    ...
    const dir = path.dirname(target.filePath);
    const currentBase = path.basename(target.filePath);
    const finalName = newFileName === currentBase ? currentBase : dedupeFileName(dir, newFileName);
    const finalPath = path.join(dir, finalName);
    try {
      if (finalPath !== target.filePath) fs.renameSync(target.filePath, finalPath);
    } catch (renameErr: any) {
      res.status(500).json({ success: false, error: `Tags saved, but renaming failed: ${renameErr.message}` });
      return;
    }
