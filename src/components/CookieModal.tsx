import { AlertCircle, CheckCircle2, Cookie, Cpu, HelpCircle, Loader2, RefreshCw, ShieldCheck, Sparkles, Trash2, Upload, X } from 'lucide-react';
...
export const CookieModal: React.FC<CookieModalProps> = ({
  ...
  const handleSave = async () => {
    if (!cookieText.trim()) {
      setFeedback({ type: 'error', message: 'Please paste cookies text first.' });
      return;
    }
    ...
    try {
      ...
      setFeedback({ type: 'error', message: err.message || 'Failed to save cookies.' });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleAutoFetch = async () => {
    ...
    try {
      ...
      setFeedback({ type: 'error', message: err.message || 'Failed to auto-fetch YouTube guest session.' });
    } finally {
  ...
  const readFile = (file: File) => {
    ...
    reader.onload = (event) => {
      ...
      if (text) {
        setCookieText(text);
        setFeedback({ type: 'success', message: `Loaded ${file.name} (${text.split('\n').length} lines). Click "Save Cookies" to apply.` });
      }
    };
  ...
  const handleClear = async () => {
    try {
      ...
      onStatusUpdated(updated);
      setTestResult(null);
      setFeedback({ type: 'success', message: 'Cookies cleared successfully.' });
    } catch {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div
        ...
        className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-2xl max-w-lg w-full max-h-[90vh] flex flex-col overflow-hidden transition-colors"
      >
        {/* Modal Header */}
        <div className="p-4 sm:p-5 border-b border-zinc-100 dark:border-zinc-800 flex items-center justify-between bg-zinc-50/50 dark:bg-zinc-950/50">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-zinc-900 dark:text-zinc-100">YouTube Session & Authentication</h3>
              <p className="text-xs text-zinc-500 dark:text-zinc-400">Node.js challenge solver & PO Token engine</p>
            </div>
          </div>
        ...
        <div className="p-4 sm:p-5 overflow-y-auto space-y-4 text-xs">
          ...
          <div className={`p-3.5 rounded-xl border flex items-center justify-between transition-colors ${
            ...
            {status.configured && (
              <button
                ...
                type="button"
                onClick={handleClear}
                className="inline-flex items-center gap-1 text-[11px] font-semibold text-rose-600 dark:text-rose-400 hover:text-rose-700 dark:hover:text-rose-300 p-1.5 rounded-lg hover:bg-rose-50 dark:hover:bg-rose-950/40 transition-colors"
              >
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <button
              ...
              className="flex items-center justify-center gap-2 p-3 rounded-xl border border-rose-200 dark:border-rose-900 bg-rose-50/50 dark:bg-rose-950/30 hover:bg-rose-50 dark:hover:bg-rose-950/50 text-rose-700 dark:text-rose-400 font-semibold transition-all shadow-xs disabled:opacity-50"
            >
              {isAutoFetching ? (
            ...
            <button
              ...
              disabled={isTesting}
              className="flex items-center justify-center gap-2 p-3 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 hover:bg-zinc-50 dark:hover:bg-zinc-800 text-zinc-700 dark:text-zinc-300 font-semibold transition-all shadow-xs disabled:opacity-50"
            >
              {isTesting ? (
                ...
                <div className="space-y-0.5">
                  ...
                  {testResult.title && (
                    <p className="text-[11px] text-zinc-600 dark:text-zinc-400">
                      Successfully verified stream parsing for: <span className="font-medium text-zinc-800 dark:text-zinc-200">{testResult.title}</span>
                    </p>
                  )}
                </div>
              </div>
          ...
          <div className="space-y-1.5">
            ...
            <div
              ...
              onDrop={handleDrop}
              className={`relative rounded-xl transition-all ${isDragging ? 'ring-2 ring-rose-500 bg-rose-50/30 dark:bg-rose-950/30' : ''}`}
            >
              <textarea
                id="cookie-textarea"
                rows={4}
                ...
                className="w-full p-3 font-mono text-[11px] rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950/60 focus:bg-white dark:focus:bg-zinc-950 focus:outline-none focus:border-rose-500 focus:ring-1 focus:ring-rose-500 text-zinc-800 dark:text-zinc-200 resize-none leading-relaxed"
              />
              {isDragging && (
                <div className="absolute inset-0 flex items-center justify-center bg-rose-500/10 rounded-xl pointer-events-none">
                  <span className="font-semibold text-rose-600 dark:text-rose-400 text-xs">Drop cookies.txt here</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="p-3 rounded-xl bg-zinc-50 dark:bg-zinc-950/60 border border-zinc-200 dark:border-zinc-800 text-zinc-600 dark:text-zinc-400 space-y-1.5">
            ...
            <ul className="space-y-1 text-[11px] leading-relaxed list-disc list-inside text-zinc-600 dark:text-zinc-400">
              <li><strong>Auto-Fetch Guest Session:</strong> Requests fresh visitor tokens and PO tokens directly from YouTube's edge API.</li>
              <li><strong>Node.js Challenge Engine:</strong> Automatically executes YouTube's JavaScript player challenges to decode audio streams.</li>
              <li><strong>Personal Account Cookies:</strong> Only required for private or age-restricted tracks that require 18+ sign-in.</li>
            </ul>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-950/50 flex items-center justify-end gap-2">
          <button
            ...
            type="button"
            onClick={onClose}
            className="px-4 py-2 rounded-lg text-xs font-semibold text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors"
          >
            Close
          </button>
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-semibold text-white bg-rose-600 hover:bg-rose-500 disabled:bg-zinc-300 dark:disabled:bg-zinc-800 dark:disabled:text-zinc-600 disabled:cursor-not-allowed transition-colors shadow-xs"
          >
            {isSaving ? (
              <>
